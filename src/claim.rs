...
use crate::{errors::{
    ...
    ERR_ZERO_CLAIM, ERR_CLAIM_FROM_TOO_MANY_STREAMS, ERR_CANT_CLAIM
}, storage::{StreamClaimResult, Segment, Status}};
...
pub trait ClaimModule: 
    ...
    fn compute_segment_value(&self, segment_start_time: u64, segment: Segment<Self::Api>) -> BigUint {
        ...
        let numerator = BigUint::from(current_time - segment_start_time).pow(segment.exponent.numerator / segment.exponent.denominator).mul(segment.amount);
        let denominator = BigUint::from(segment.duration).pow(segment.exponent.numerator / segment.exponent.denominator);
    ...
    fn sender_balance(&self, stream_id: u64) -> BigUint {
     ...
     fn claim_from_stream(
        ...
        let is_warm = self.is_warm(stream_id);
         ...
         StreamClaimResult { stream_id, recipient: stream.recipient, payment_token: stream.payment_token, payment_nonce: stream.payment_nonce, claimed_amount: amount, is_finalized }
     }
 
     /// Allow claim from multiple streams in one transaction
     /// Note: Invalid stream IDs are ignored
     #[endpoint(claimFromStreamMultiple)]
     fn claim_from_stream_multiple(
         &self,
         stream_ids: ManagedVec<u64>
     ) -> ManagedVec<StreamClaimResult<Self::Api>> {
         require!(stream_ids.len() <= MAX_CLAIMS_PER_TX, ERR_CLAIM_FROM_TOO_MANY_STREAMS);
         
         let mut stream_claim_results = ManagedVec::new();
 
         for stream_id in &stream_ids {
             if self.stream_by_id(stream_id).is_empty() == false {
                 let stream_claim_result = self.claim_from_stream(stream_id);
                 stream_claim_results.push(stream_claim_result);
             }
         }
 
         stream_claim_results
     }
