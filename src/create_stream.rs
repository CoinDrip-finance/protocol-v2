use crate::{errors::{
    ...
    ERR_ZERO_DEPOSIT,
    ERR_START_TIME,
    ERR_END_TIME, ERR_CLIFF_TOO_BIG, ERR_BROKER_FEE_TOO_BIG
}, storage::{Stream, BrokerFee, Segment, Exponent}};
...
pub trait CreateStreamModule: 
    ...
    fn create_stream_duration(
        ...
        self.create_stream(recipient, start_time, end_time, cliff_opt, can_cancel_opt, broker_opt)
    }
    ...
    fn create_stream(
        ...
        if !self.protocol_fee(&token_identifier).is_empty() {
            let protocol_fee = self.protocol_fee(&token_identifier).get().mul(stream_amount.clone()).div(100_00u32);
            self.send().direct(&self.blockchain().get_owner_address(), &token_identifier, token_nonce, &protocol_fee);
        ...
        if broker_opt.is_some() {
            ...
            if broker.fee > BigUint::zero() {
                ...
                self.send().direct(&broker.address, &token_identifier, token_nonce, &broker_fee);
        ...
        self.create_stream_event(stream_id, &caller, &recipient, &token_identifier, token_nonce, &stream_amount, start_time, end_time);
